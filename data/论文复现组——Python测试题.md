...
**快速幂**:也称为二进制取幂，是一种在O(log n)的时间复杂度内计算a^n （其中n是一个非常大的整数）的算法。它通过将指数n进行二进制拆分，并利用幂的运算性质，极大地减少了计算所需的乘法次数。
例如 3^10，可以根据指数10的二进制形式1010，将指数拆分为(2^3+2^1),即8+2，将3^10化为3^(2^3)*3^(2^1).
